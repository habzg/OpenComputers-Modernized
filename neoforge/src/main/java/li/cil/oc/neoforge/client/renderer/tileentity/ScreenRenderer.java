...
public class ScreenRenderer implements BlockEntityRenderer<Screen> {
    ...
    public void render(Screen screen, float partialTick, @NotNull PoseStack poseStack,
        ...
        if (player != null && !player.getMainHandItem().isEmpty()) {
            if (screen.facing() == Direction.UP || screen.facing() == Direction.DOWN) {
                ...
                boolean isScreenBlock = info != null && info.block() instanceof li.cil.oc.neoforge.common.block.Screen;
    ...
    @Override
    public net.minecraft.world.phys.@NotNull AABB getRenderBoundingBox(Screen blockEntity) {
    ...
    private void renderScreenContent(Screen screen, float partialTick, PoseStack poseStack,
        ...
        double dx = (screen.getBlockPos().getX() + 0.5) - eyePos.x;
        double dy = (screen.getBlockPos().getY() + 0.5) - eyePos.y;
        double dz = (screen.getBlockPos().getZ() + 0.5) - eyePos.z;

        if (screenFacing.getStepX() * dx + screenFacing.getStepY() * dy + screenFacing.getStepZ() * dz < -1.5) return;

        double distance = Math.sqrt(dx * dx + dy * dy + dz * dz) / Math.min(screen.width, screen.height);
        double maxDist = Settings.get().maxScreenTextRenderDistance;
        double fadeStart = Settings.get().screenTextFadeStartDistance;
        ...
        poseStack.translate(2.25f / 16.0f / scale, 2.25f / 16.0f / scale, 0.01f);
