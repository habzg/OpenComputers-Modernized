...
public class Printer extends DynamicGuiContainer<li.cil.oc.neoforge.common.container.Printer> {
    ...
    public Printer(Inventory playerInventory, li.cil.oc.core.impl.common.tileentity.Printer printer) {
        super(new li.cil.oc.neoforge.common.container.Printer(0, playerInventory, printer, playerInventory.player));
    ...
    public Printer(li.cil.oc.neoforge.common.container.Printer container, Inventory inv, Component title) {
        super(container, inv, title);
        this.printer = (li.cil.oc.core.impl.common.tileentity.Printer) container.otherInventory;
        imageWidth = 176;
        imageHeight = 166;
        materialBar = addWidget(new ProgressBar(40, 21) {
            @Override
    ...
    protected void drawSecondaryForegroundLayer(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.drawString(font, Component.translatable(printer.getInventoryName()).getString(), 8, 6, 0x404040);
        if (isHovering(materialBar.x, materialBar.y, materialBar.width(), materialBar.height(), mouseX, mouseY)) {
            var tooltip = new ArrayList<Component>();
            tooltip.add(Component.literal(menu.amountMaterial() + "/" + li.cil.oc.core.impl.common.tileentity.Printer.MAX_AMOUNT_MATERIAL));
        ...
        if (isHovering(inkBar.x, inkBar.y, inkBar.width(), inkBar.height(), mouseX, mouseY)) {
            var tooltip = new ArrayList<Component>();
            tooltip.add(Component.literal(menu.amountInk() + "/" + li.cil.oc.core.impl.common.tileentity.Printer.MAX_AMOUNT_INK));
            guiGraphics.renderTooltip(font, tooltip, java.util.Optional.empty(), mouseX - leftPos, mouseY - topPos);
        }
    }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float dt, int mouseX, int mouseY) {
        ...
        materialBar.level = (double) menu.amountMaterial() / li.cil.oc.core.impl.common.tileentity.Printer.MAX_AMOUNT_MATERIAL;
        inkBar.level = (double) menu.amountInk() / li.cil.oc.core.impl.common.tileentity.Printer.MAX_AMOUNT_INK;
