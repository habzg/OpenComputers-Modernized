...
public abstract class Agent extends AgentBase {
    @Override
    protected li.cil.oc.neoforge.server.agent.@NotNull Player createRotatedPlayer(@NotNull Direction facing, @NotNull Direction side) {
    ...
    protected boolean playerPlaceBlock(@NotNull Player player, int slot, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        return ((li.cil.oc.neoforge.server.agent.Player) player).placeBlock(slot, x, y, z, side, hitX, hitY, hitZ);
    }
