...
import li.cil.oc.core.impl.server.component.ServerBase;
...
public class Server extends ServerBase {
    ...
    public int tier() {
        ...
        if (item.getItem() instanceof li.cil.oc.neoforge.common.item.Server serverItem) {
    ...
    public boolean onActivate(@NotNull Player player, InteractionHand hand, ItemStack heldItem, float hitX, float hitY) {
        if (!rack.level().isClientSide) {
            if (player.isShiftKeyDown()) {
                ...
                player.openMenu(OpenComputers.getContainerProvider(GuiType.ServerInRack, rack.level(), position.x(), GuiType.embedSlot(position.y(), this.slot), position.z()), (net.minecraft.network.RegistryFriendlyByteBuf buf) -> {
                    buf.writeInt(GuiType.ServerInRack);
                    buf.writeInt(position.x());
                    buf.writeInt(GuiType.embedSlot(position.y(), this.slot));
