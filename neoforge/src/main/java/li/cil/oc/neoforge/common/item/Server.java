...
public class Server extends DelegateItem implements ItemTier {
    ...
    public void appendHoverText(@NotNull ItemStack stack, @NotNull TooltipContext context, @NotNull List<Component> tooltip, @NotNull TooltipFlag flag) {
        ...
        if (flag.isAdvanced() && customData != null && !customData.isEmpty()) {
            var tag = customData.copyTag();
            if (tag.contains("oc:data")) {
                var data = tag.getCompound("oc:data");
                if (data.contains("items")) {
                    var items = data.getList("items", net.minecraft.nbt.Tag.TAG_COMPOUND);
                    if (!items.isEmpty()) {
                        tooltip.add(Component.literal(""));
                        tooltip.add(Component.translatable("tooltip.opencomputers.server.Components"));
    ...
    public @NotNull InteractionResultHolder<ItemStack> use(@NotNull Level level, Player player, @NotNull InteractionHand hand) {
        ...
        if (!player.isShiftKeyDown()) {
            if (!level.isClientSide) {
                player.openMenu(OpenComputers.getContainerProvider(GuiType.Server, level, 0, 0, 0), (net.minecraft.network.RegistryFriendlyByteBuf buf) -> {
                    buf.writeInt(GuiType.Server);
                    buf.writeInt(0);
                    buf.writeInt(0);
                    buf.writeInt(0);
                });
