...
public class Manual extends Screen {
    ...
    public int offset() {
        return li.cil.oc.neoforge.client.Manual.INSTANCE.history.getFirst().offset;
    }
    ...
    public void refreshPage() {
        var entry = li.cil.oc.neoforge.client.Manual.INSTANCE.history.getFirst();
        var maybeContent = li.cil.oc.neoforge.client.Manual.INSTANCE.contentFor(entry.path);
    ...
    public void pushPage(String path) {
        if (!path.equals(li.cil.oc.neoforge.client.Manual.INSTANCE.history.getFirst().path)) {
            li.cil.oc.neoforge.client.Manual.INSTANCE.history.push(new li.cil.oc.neoforge.client.Manual.History(path));
            refreshPage();
        }
    }

    public void popPage() {
        if (li.cil.oc.neoforge.client.Manual.INSTANCE.history.size() > 1) {
            li.cil.oc.neoforge.client.Manual.INSTANCE.history.pop();
            refreshPage();
        } else {
    ...
    public void actionPerformed(ImageButton button) {
        if (button.getId() >= 0 && button.getId() < li.cil.oc.neoforge.client.Manual.INSTANCE.tabs.size()) {
            li.cil.oc.neoforge.client.Manual.INSTANCE.navigate(li.cil.oc.neoforge.client.Manual.INSTANCE.tabs.get(button.getId()).path());
        }
    }
    ...
    public void init() {
        ...
        leftPos = (width - 256) / 2;
        topPos = (height - 192) / 2;
        for (int i = 0; i < Math.min(li.cil.oc.neoforge.client.Manual.INSTANCE.tabs.size(), maxTabsPerSide); i++) {
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float dt) {
        ...
        for (int i = 0; i < Math.min(li.cil.oc.neoforge.client.Manual.INSTANCE.tabs.size(), maxTabsPerSide); i++) {
            ImageButton button = (ImageButton) renderables.get(i);
            guiGraphics.pose().pushPose();
            guiGraphics.pose().translate(button.getX() + 5, button.getY() + 5, 0);
            li.cil.oc.neoforge.client.Manual.INSTANCE.tabs.get(i).renderer().render(guiGraphics);
            guiGraphics.pose().popPose();
        }
        Document.setCurrentPagePath(li.cil.oc.neoforge.client.Manual.INSTANCE.history.getFirst().path);
        int clipX = leftPos + 7;
        int clipY = topPos + 7;
        ...
        if (!isDragging) {
            for (int i = 0; i < Math.min(li.cil.oc.neoforge.client.Manual.INSTANCE.tabs.size(), maxTabsPerSide); i++) {
                ...
                if (mouseX > button.getX() && mouseX < button.getX() + tabWidth && mouseY > button.getY() && mouseY < button.getY() + tabHeight) {
                    String tabTooltip = li.cil.oc.neoforge.client.Manual.INSTANCE.tabs.get(i).tooltip();
                    if (tabTooltip != null)
    ...
    private void scrollTo(int row) {
        li.cil.oc.neoforge.client.Manual.INSTANCE.history.getFirst().offset = Math.clamp(row, 0, maxOffset());
