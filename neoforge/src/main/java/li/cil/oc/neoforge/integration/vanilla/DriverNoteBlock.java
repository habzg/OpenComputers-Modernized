...
public final class DriverNoteBlock extends DriverSidedBlock {
    ...
    public ManagedEnvironment createEnvironment(Level world, int x, int y, int z, Direction side) {
        return new Environment(world, new BlockPos(x, y, z));
    }
    ...
    public static final class Environment extends li.cil.oc.api.prefab.ManagedEnvironment implements NamedBlock {
        ...
        public Object[] trigger(Context context, Arguments args) {
            ...
            float pitch = NoteBlock.getPitchFromNote(state.getValue(NoteBlock.NOTE));
            world.playSeededSound(null, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
                    state.getValue(NoteBlock.INSTRUMENT).getSoundEvent().value(),
                    SoundSource.RECORDS, NoteBlock.NOTE_VOLUME, pitch, world.random.nextLong());
