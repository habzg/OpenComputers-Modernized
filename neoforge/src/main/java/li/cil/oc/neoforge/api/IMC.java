package li.cil.oc.neoforge.api;
 ...
 * only be registered via OpenComputers' registration API.
 * <br>
 * Use this if you have some kind of abstraction layer in place anyway, and can
 * safely use the API without class not found exceptions and such.
 * <br>
 * This also serves to document all registration messages OpenComputers
 * handles.
 * <br>
 ...
 * class gets updated.
 * <br>
 * Note: on NeoForge this class exists on the classpath of the running game
 * and delegates directly to OpenComputers' internal registrar. Register
 * things during mod initialization, <em>not</em> after OC has finished its
 * own initialization, or the registration will be rejected.
 */
...
public final class IMC {
    ...
    public static void registerAssemblerFilter(final String callback) {
        Registrar.registerAssemblerFilter(callback);
    }
    ...
    public static void registerAssemblerTemplate(final String name, final String select, final String validate, final String assemble, final Class host, final int[] containerTiers, final int[] upgradeTiers, final Iterable<Pair<String, Integer>> componentSlots) {
        Registrar.registerAssemblerTemplate(name, select, validate, assemble, host, containerTiers, upgradeTiers, componentSlots);
    }
    ...
    public static void registerDisassemblerTemplate(final String name, final String select, final String disassemble) {
        Registrar.registerDisassemblerTemplate(name, select, disassemble);
    }
    ...
    public static void registerToolDurabilityProvider(final String callback) {
        Registrar.registerToolDurabilityProvider(callback);
    }
     ...
     * Signature of callbacks must be:
     * <pre>
     * boolean callback(Player player, int x, int y, int z, boolean changeDurability)
     * </pre>
     * <br>
     * Callbacks must be declared as {@code packagePath.className.methodName}.
     * For example: {@code com.example.Integration.callbackMethod}.
    ...
    public static void registerWrenchTool(final String callback) {
        Registrar.registerWrenchTool(callback);
    }
    ...
    public static void registerWrenchToolCheck(final String callback) {
        Registrar.registerWrenchToolCheck(callback);
    }
    ...
    public static void registerItemCharge(final String name, final String canCharge, final String charge) {
        Registrar.registerItemCharge(name, canCharge, charge);
    }
    ...
    public static void registerInkProvider(final String callback) {
        Registrar.registerInkProvider(callback);
    }
    ...
    public static void blacklistPeripheral(final Class peripheral) {
        Registrar.blacklistPeripheral(peripheral);
    }
    ...
    public static void blacklistHost(final String name, final Class<?> host, final ItemStack stack) {
        Registrar.blacklistHost(name, host, stack);
    }
    ...
    public static void registerProgramDiskLabel(final String programName, final String diskLabel, final String... architectures) {
        Registrar.registerProgramDiskLabel(programName, diskLabel, architectures);
    }
