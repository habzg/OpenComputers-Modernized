...
import li.cil.oc.neoforge.event.RobotRenderEventImpl;
...
public class RobotRenderer implements BlockEntityRenderer<RobotProxy> {
    public static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(Settings.resourceDomain, "textures/model/robot.png");
    ...
    public void render(@NotNull RobotProxy proxy, float partialTick, @NotNull PoseStack poseStack, @NotNull MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        renderNameLabel(robot, robot, poseStack, buffer, packedLight);
    ...
    private void renderChassis(Robot robot, Robot proxy, float partialTick, double offset, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        var event = new RobotRenderEventImpl(robot, mountPoints);
        ...
        if (robot != null && !robot.renderingErrored) {
            li.cil.oc.core.impl.client.renderer.item.UpgradeRenderer.setModelTint(saveR, saveG, saveB);
    ...
    private void renderTool(Robot robot, Robot proxy, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        try {
            ...
            if (item instanceof BlockItem) {
                ...
                poseStack.mulPose(new Quaternionf().rotateZ((float) Math.toRadians(10)));
                poseStack.scale(0.625F, -0.625F, 0.625F);
                ...
                poseStack.translate(0.0625F, -0.125F, -2.0f / 16.0f);
                poseStack.scale(0.625F, -0.625F, 0.625F);
            } else {
    ...
    private void renderUpgrades(Robot robot, Robot proxy, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        for (int i = 0; i < slotMapping.size(); i++) {
            ...
            if (info != null) {
                try {
                    ...
                    renderer.render(stack, mountPoint, proxy, partialTick, poseStack, buffer, packedLight, packedOverlay);
                    poseStack.popPose();
                } catch (Exception e) {
    ...
    private void renderNameLabel(Robot robot, Robot proxy, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        String name = robot.info.name;
        if (!Settings.get().robotLabels || name == null || name.isEmpty()) return;
        ...
        font.drawInBatch(displayName, f2, 0, -1, false, matrix, buffer, Font.DisplayMode.SEE_THROUGH, j, packedLight);
