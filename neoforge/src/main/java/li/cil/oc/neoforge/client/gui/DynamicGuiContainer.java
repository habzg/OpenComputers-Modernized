...
import li.cil.oc.core.impl.client.gui.CustomGuiContainer;
import li.cil.oc.neoforge.common.container.ComponentSlot;
import li.cil.oc.neoforge.common.container.Player;
...
public abstract class DynamicGuiContainer<C extends AbstractContainerMenu> extends CustomGuiContainer<C> {
    ...
    protected void drawSecondaryForegroundLayer(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        guiGraphics.drawString(font, Component.translatable("container.inventory").getString(), 8, imageHeight - 96 + 2, 0x404040);
    }
    ...
    protected void renderSlot(@NotNull GuiGraphics guiGraphics, @NotNull Slot slot) {
        ...
        if (!slot.hasItem() && slot instanceof ComponentSlot cs) {
            ...
            var slotIcon = li.cil.oc.core.impl.client.gui.Icons.get(cs.slot());
            if (slotIcon != null) {
