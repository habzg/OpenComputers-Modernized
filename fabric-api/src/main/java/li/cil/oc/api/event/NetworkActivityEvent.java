 ...
 */
public class NetworkActivityEvent implements Cancelled {
    ...
    public static final class Client extends NetworkActivityEvent {
        ...
        @FunctionalInterface
        public interface Listener {
            void onNetworkActivityClient(Client event);
        }

        public static final Event<Listener> EVENT = EventFactory.createArrayBacked(Listener.class, (listeners) -> (event) -> {
            for (Listener listener : listeners) {
                listener.onNetworkActivityClient(event);
            }
        });
    }
}
