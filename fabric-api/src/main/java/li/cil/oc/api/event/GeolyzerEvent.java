 ...
 * <br>
 * When canceling this event, the respective method will bail and report
 * that the operation failed.
 */
public abstract class GeolyzerEvent implements Cancelled {
    ...
    public static class Analyze extends GeolyzerEvent {
        ...
        @FunctionalInterface
        public interface Listener {
            void onGeolyzerAnalyze(Analyze event);
        }

        public static final Event<Listener> EVENT = EventFactory.createArrayBacked(Listener.class, (listeners) -> (event) -> {
            for (Listener listener : listeners) {
                listener.onGeolyzerAnalyze(event);
                if (event.isCanceled()) break;
            }
        });
    }
