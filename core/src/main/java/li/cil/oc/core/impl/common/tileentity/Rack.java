...
public class Rack extends HubTileEntity implements PowerAcceptor, PowerBalancer, ComponentInventory, Rotatable, BundledRedstoneAware, Analyzable, li.cil.oc.api.internal.Rack, li.cil.oc.api.util.StateAware {
    ...
    public Node[] onAnalyze(Player player, int side, float hitX, float hitY, float hitZ) {
        ...
        var slot = slotAt(dir, hitX, hitY, hitZ);
        if (slot.isPresent()) {
    ...
    private static final String FacingTag = Settings.namespace + "facing";
    ...
    public void writeToNBTForClient(CompoundTag nbt) {
        ...
        var provider = getEffectiveProvider();
        if (provider != null) {
            var wasSavingForClients = li.cil.oc.core.impl.common.tileentity.traits.TileEntity.savingForClients;
            li.cil.oc.core.impl.common.tileentity.traits.TileEntity.savingForClients = true;
            try {
                save(nbt, provider);
            } finally {
                li.cil.oc.core.impl.common.tileentity.traits.TileEntity.savingForClients = wasSavingForClients;
            }
        }
    }
