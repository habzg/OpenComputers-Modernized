...
public interface TankWorldControl extends TankAware, WorldAware, SideRestricted {
    @Callback(doc = "function(side:number [, tank:number]):boolean -- Compare the fluid in the selected tank with the fluid in the specified tank on the specified side.")
    default Object[] compareFluid(Context context, Arguments args) {
        ...
        if (stack != null) {
            FluidHandler handler = FluidUtils.fluidHandlerAt(position().offset(side));
            if (handler != null) {
    ...
    @Callback(doc = "function(side:number[, amount:number=1000]):boolean, number or string -- Drains the specified amount of fluid from the specified side.")
    default Object[] drain(Context context, Arguments args) {
        ...
        FluidHandler handler = FluidUtils.fluidHandlerAt(position().offset(facing));
        if (handler == null) return ResultWrapper.result(null, "incompatible or no fluid");
        FluidStack existing = tank.getFluid();
        FluidStack drained = handler.drain(existing != null ? existing.copyWithAmount(amount) : new FluidStack("", 0), false);
    ...
    @Callback(doc = "function(side:number[, amount:number=1000]):boolean, number or string -- Eject the specified amount of fluid to the specified side.")
