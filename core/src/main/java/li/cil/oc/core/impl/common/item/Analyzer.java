...
public class Analyzer extends DelegateItem {
    ...
    private static void analyzeNodes(Node[] nodes, Player player) {
        ...
        if (fakePlayerCheck.test(player)) return;
        for (Node node : nodes) {
            if (node == null) continue;
            if (node.host() instanceof Machine machine) {
                if (machine.lastError() != null) {
                    player.sendSystemMessage(Component.translatable("gui.opencomputers.analyzer.lasterror", Component.translatable(machine.lastError())));
                }
                player.sendSystemMessage(Component.translatable("gui.opencomputers.analyzer.components", machine.componentCount() + "/" + machine.maxComponents()));
                ...
                if (users.length != 0) {
                    ...
                    player.sendSystemMessage(Component.translatable("gui.opencomputers.analyzer.users", sb.toString()));
                }
            }
            if (node instanceof Connector connector) {
                if (connector.localBufferSize() > 0) {
                    player.sendSystemMessage(Component.translatable("gui.opencomputers.analyzer.storedenergy", String.format("%.2f/%.2f", connector.localBuffer(), connector.localBufferSize())));
                }
            ...
            if (address != null && !address.isEmpty()) {
                net.minecraft.network.chat.MutableComponent addr = Component.translatable("gui.opencomputers.analyzer.address", address);
                ...
                addr.setStyle(addr.getStyle().withHoverEvent(new net.minecraft.network.chat.HoverEvent(net.minecraft.network.chat.HoverEvent.Action.SHOW_TEXT, Component.translatable("gui.opencomputers.analyzer.copytoclipboard"))));
    ...
    public @NotNull InteractionResult useOn(UseOnContext context) {
            ...
            if (player != null && player.isShiftKeyDown()) {
                ...
                if (cd != null && !cd.isEmpty()) {
                    ...
                    if (tag.contains(OCSettings.namespace + "clipboard")) {
                        if (!world.isClientSide) {
                            screen.origin.buffer().clipboard(tag.getString(OCSettings.namespace + "clipboard"), player);
                        }
        ...
        if (te != null && player != null) {
            analyze(te, player, side, hitX, hitY, hitZ);
        }
        return InteractionResult.SUCCESS;
    }
}
