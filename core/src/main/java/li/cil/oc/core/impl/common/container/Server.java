...
public class Server extends Player {
    public final li.cil.oc.core.impl.server.component.Server server;
    public boolean isRunning = false;
    public boolean isItem = true;
    public BlockPos rackPos = null;
    public int rackSlot = -1;

    public Server(MenuType<?> menuType, int containerId, Inventory playerInventory, ServerInventory serverInventory, li.cil.oc.core.impl.server.component.Server server, net.minecraft.world.entity.player.Player player) {
        super(menuType, containerId, playerInventory, serverInventory, player);
        this.server = server;
    ...
    public void updateCustomData(CompoundTag nbt) {
        ...
        if (nbt.contains("rackPosX")) {
            rackPos = new BlockPos(nbt.getInt("rackPosX"), nbt.getInt("rackPosY"), nbt.getInt("rackPosZ"));
