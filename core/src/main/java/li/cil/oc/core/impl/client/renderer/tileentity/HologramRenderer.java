...
public class HologramRenderer implements BlockEntityRenderer<Hologram> {
    ...
    public void render(@NotNull Hologram hologram, float partialTick, @NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        double dx = (hologram.getBlockPos().getX() + 0.5) - camera.getPosition().x;
        double dy = (hologram.getBlockPos().getY() + 0.5) - camera.getPosition().y;
        double dz = (hologram.getBlockPos().getZ() + 0.5) - camera.getPosition().z;
        double playerDistSq = dx * dx + dy * dy + dz * dz;

        double maxScale = Arrays.stream(Settings.get().hologramMaxScaleByTier).max().orElse(4.0);
        double maxDistSq = hologram.scale / maxScale * Settings.get().hologramRenderDistance * Settings.get().hologramRenderDistance;
        double fadeDistSq = hologram.scale / maxScale * Settings.get().hologramFadeStartDistance * Settings.get().hologramFadeStartDistance;
