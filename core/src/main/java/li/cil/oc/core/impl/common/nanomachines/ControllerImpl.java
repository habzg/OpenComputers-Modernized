...
import li.cil.oc.core.impl.integration.util.DamageSourceWithRandomCause;
...
public class ControllerImpl implements Controller, WirelessEndpoint {
  ...
  public double storedEnergy = OCSettings.get().bufferNanomachines * 0.25;
  ...
  public ControllerImpl(Player player) {
    ...
    this.CommandRange = OCSettings.get().nanomachinesCommandRange * OCSettings.get().nanomachinesCommandRange;
  }
      ...
      if (dSq <= CommandRange && packet.data().length > 0 && packet.data()[0] instanceof byte[] header &&
        ...
        if (cmd.length > 0) {
          ...
          switch (op) {
            ...
            case "getAge" ->
              respond(sender, "age", player instanceof net.minecraft.server.level.ServerPlayer sp ? (int) ((System.currentTimeMillis() - sp.getLastActionTime()) / 1000) : player.tickCount / 20);
  ...
  public void respond(WirelessEndpoint endpoint, Object... data) {
    queuedCommand = () -> {
      if (responsePort > 0) {
        double cost = OCSettings.get().wirelessCostPerRange[Tier.Two] * CommandRange;
        double epsilon = 0.1;
    ...
    commandDelay = (int) (OCSettings.get().nanomachinesCommandDelay * 20);
  }
  ...
  public Controller reconfigure() {
    if (isServer()) synchronized (configuration) {
      ...
      if (player instanceof ServerPlayer sp) {
        ...
        changeBuffer(-OCSettings.get().nanomachineReconfigureCost);
  ...
  public int getSafeActiveInputs() {
    return OCSettings.get().nanomachinesSafeInputsActive;
  }

  public int getMaxActiveInputs() {
    return OCSettings.get().nanomachinesMaxInputsActive;
  }
  ...
  public boolean setInput(int index, boolean value) {
    ...
    synchronized (configuration) {
      if (!value || configuration.triggers.stream().filter(t -> t.isActive).count() < OCSettings.get().nanomachinesMaxInputsActive) {
  ...
  public double getLocalBufferSize() {
    return OCSettings.get().bufferNanomachines;
  }
  ...
  public void update() {
    ...
    boolean hasPower = getLocalBuffer() > 0 || OCSettings.get().ignorePower;

    if (hasPower != hadPower) {
      if (!hasPower) {
        for (Behavior b : getActiveBehaviors()) b.onDisable(DisableReason.OutOfEnergy);
        hasPower = getLocalBuffer() > 0 || OCSettings.get().ignorePower;
      } else {
    ...
    if (hasPower) {
      ...
      if (isServer()) {
        if (player.level().getGameTime() % OCSettings.get().tickFrequency == 0) {
          long activeInputs = configuration.triggers.stream().filter(t -> t.isActive).count();
          changeBuffer(-OCSettings.get().nanomachineCost * OCSettings.get().tickFrequency * (activeInputs + 0.5));
      ...
      if (isClient() && OCSettings.get().enableNanomachinePfx) {
