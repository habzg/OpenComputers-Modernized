...
public interface Network {
     ...
     * <br>
     * This does <em>not</em> include nodes with a visibility of <code>None</code>
     * or a visibility of <code>Neighbors</code> when there is no direct connection
     ...
     * <code>component_added</code> signals for all visible components in the
     * network.
     *
     ...
     * <br>
     * This <em>does</em> include nodes with a visibility of <code>None</code>.
     * <br>
     * This does <em>not</em> include the node itself.
     * <br>
     ...
     * <br>
     * If the target node with that address has a visibility of <code>None</code>
     * the message will <em>not</em> be delivered to that node. If the target
     * node with that address has a visibility of <code>Neighbors</code> and the
     ...
     * messages named <code>computer.signal</code> by converting the message data to
     * a signal and inject that signal into the machine, so no message not used
     * for this purpose should be named <code>computer.signal</code>.
     *
