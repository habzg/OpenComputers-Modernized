...
public final class ClientPacketHandler extends CommonPacketHandler {
    ...
    private final java.util.Map<Player, Long> lastDropTimes = new java.util.HashMap<>();
    ...
    public void dispatch(PacketParser p) {
        switch (p.packetType) {
            ...
            case HologramTranslation -> onHologramTranslation(p);
    ...
    private void onAdapterState(PacketParser p) {
        try {
            ...
            if (t != null) {
                ...
                if (level != null) {
                    level.sendBlockUpdated(t.getBlockPos(), t.getBlockState(), t.getBlockState(), 3);
                }
            }
        } catch (IOException e) {
    ...
    private void onClientLog(PacketParser p) {
        try {
            var log = org.slf4j.LoggerFactory.getLogger("OpenComputers-Client");
            log.info(p.readUTF());
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    private void onDropFile(PacketParser p) {
        try {
            ...
            if (fileName.isEmpty() || fileName.contains("..") || fileName.contains("/") || fileName.contains("\\")) {
                org.slf4j.LoggerFactory.getLogger("OpenComputers-Client").warn("Invalid drop file name: {}", fileName);
                return;
            }
            long now = System.currentTimeMillis();
            Long lastDrop = lastDropTimes.get(p.player);
            if (lastDrop != null && now - lastDrop < 1000) {
                org.slf4j.LoggerFactory.getLogger("OpenComputers-Client").warn("Drop file rate limit exceeded for player {}", p.player.getName().getString());
                return;
            }
            lastDropTimes.put(p.player, now);
            int fileSize = p.readInt();
            if (fileSize <= 0 || fileSize > MAX_DROP_FILE_SIZE) {
                org.slf4j.LoggerFactory.getLogger("OpenComputers-Client").warn("Invalid drop file size: {}", fileSize);
                return;
            }
    ...
    private void onFileSystemActivity(PacketParser p) {
        try {
            ...
            if (p.readBoolean()) {
                ...
                if (t != null) {
                    FileSystemAccessEvent.Client.EVENT.invoker().onFileSystemAccessClient(new FileSystemAccessEvent.Client(sound, t, data));
                }
            } else {
                ...
                FileSystemAccessEvent.Client.EVENT.invoker().onFileSystemAccessClient(new FileSystemAccessEvent.Client(sound, w, x, y, z, data));
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    private void onNetworkActivity(PacketParser p) {
        try {
            ...
            if (p.readBoolean()) {
                net.minecraft.world.level.block.entity.BlockEntity t = p.readBlockEntity(net.minecraft.world.level.block.entity.BlockEntity.class);
                if (t != null) {
                    NetworkActivityEvent.Client.EVENT.invoker().onNetworkActivityClient(new NetworkActivityEvent.Client(t, data));
                }
            } else {
                Level w = world(p.player, p.readInt());
                double x = p.readDouble();
                double y = p.readDouble();
                double z = p.readDouble();
                NetworkActivityEvent.Client.EVENT.invoker().onNetworkActivityClient(new NetworkActivityEvent.Client(w, x, y, z, data));
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    ...
    private void onHologramColor(PacketParser p) {
        try {
            ...
            if (t != null) {
                int index = p.readInt();
                int value = p.readInt();
                if (index < 0 || index >= t.colors.length) return;
    ...
    private void onHologramTranslation(PacketParser p) {
        try {
            Hologram t = p.readBlockEntity(Hologram.class);
            if (t != null) {
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (vx * vx + vy * vy + vz * vz < 1) {
                    ...
                    w.addParticle(java.util.Objects.requireNonNullElseGet(particle, () -> new DustParticleOptions(new Vector3f(1.0f, 0.0f, 0.0f), 1.0f)), px, py, pz, vx, vy + velocity * 0.25, vz);
                }
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
    ...
    private void onRackMountableData(PacketParser p) {
        try {
            ...
            if (t != null) {
                ...
                if (mountableIndex < 0 || mountableIndex >= t.lastData.length) return;
                var data = p.readNBT();
    ...
    private void onRobotAnimateSwing(PacketParser p) {
        try {
            var t = p.readBlockEntity(RobotProxy.class);
            if (t != null) {
    ...
    private void onRobotInventoryChange(PacketParser p) {
        try {
            ...
            if (t != null) {
                int slot = p.readInt();
                var stack = p.readItemStack();
                if (slot < 0 || slot >= t.getContainerSize()) return;
                    ...
                    if (comps != null && slot < comps.length && comps[slot] != null && !stack.isEmpty()) {
                        ...
                        if (driver != null) {
                            try {
                                ...
                                org.slf4j.LoggerFactory.getLogger("OpenComputers-Client").warn("Failed to reload component in slot {} from sync data", slot, e);
                            }
                        }
                    }
                } else {
    ...
    private void onRobotMove(PacketParser p) {
        try {
            ...
            int z = p.readInt();
            Direction direction = p.readDirection();
            var t = p.getBlockEntity(dimension, x, y, z, RobotProxy.class);
            if (t != null && direction != null) {
                t.move(direction);
            } else if (direction != null) {
                var oldPos = new net.minecraft.core.BlockPos(x, y, z);
                var newPos = oldPos.relative(direction);
                var newProxy = p.getBlockEntity(dimension, newPos.getX(), newPos.getY(), newPos.getZ(), RobotProxy.class);
                if (newProxy != null) {
    ...
    private void onRotatableState(PacketParser p) {
        ...
        if (t != null) {
            ...
            if (pitch != null && yaw != null && t instanceof Screen screen) {
    ...
    private void onTextBufferPowerChange(PacketParser p) {
        try {
            ManagedEnvironment env = ClientComponentTracker.INSTANCE.get(p.player.level(), p.readUTF());
    ...
    private void onOpenTabletTerminal(PacketParser p) {
        ...
        host.update();
        PacketSender.sendMachineItemStateRequest(stack);
        for (ManagedEnvironment env : host.componentEnvironments()) {
            if (env instanceof li.cil.oc.core.impl.common.component.TextBuffer buffer) {
                if (buffer.proxy.nodeAddress.isEmpty() && !address.isEmpty()) {
                    ...
                    if (level != null) {
                        ClientComponentTracker.INSTANCE.add(level, address, buffer);
                    }
        ...
        for (ManagedEnvironment env : host.componentEnvironments()) {
            if (env instanceof TextBuffer buffer) {
                ...
                Minecraft.getInstance().setScreen(new net.minecraft.client.gui.screens.Screen(Component.literal("tablet")) {
                    @Override
    ...
    private void onTextBufferInit(PacketParser p) {
        try {
            ManagedEnvironment bufEnv = ClientComponentTracker.INSTANCE.get(p.player.level(), p.readUTF());
    ...
    private void onTextBufferMulti(PacketParser p) {
        try {
            ...
            ManagedEnvironment bufEnv2 = ClientComponentTracker.INSTANCE.get(p.player.level(), addr);
            if (bufEnv2 instanceof TextBuffer buffer) {
                multiLoop:
                while (true) {
                    ...
                    switch (type) {
                        ...
                        case TextBufferMultiViewportResolutionChange -> onTextBufferMultiViewportResolutionChange(p, buffer);
    ...
    private void onTextBufferMultiColorChange(PacketParser p, TextBuffer buffer) {
        try {
    ...
    private void onTextBufferMultiCopy(PacketParser p, TextBuffer buffer) {
        try {
            int col = p.readInt();
            int row = p.readInt();
            int w = p.readInt();
    ...
    private void onTextBufferMultiDepthChange(PacketParser p, TextBuffer buffer) {
        try {
            buffer.setColorDepth(TextBuffer.ColorDepth.values()[p.readInt()]);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }

    private void onTextBufferMultiFill(PacketParser p, TextBuffer buffer) {
        try {
            int col = p.readInt();
            int row = p.readInt();
            int w = p.readInt();
    ...
    private void onTextBufferMultiPaletteChange(PacketParser p, TextBuffer buffer) {
        try {
            int index = p.readInt();
            int color = p.readInt();
    ...
    private void onTextBufferMultiResolutionChange(PacketParser p, TextBuffer buffer) {
        try {
            int w = p.readInt();
            int h = p.readInt();
    ...
    private void onTextBufferMultiViewportResolutionChange(PacketParser p, TextBuffer buffer) {
        try {
            int w = p.readInt();
            int h = p.readInt();
    ...
    private void onTextBufferMultiMaxResolutionChange(PacketParser p, TextBuffer buffer) {
        try {
            int w = p.readInt();
            int h = p.readInt();
    ...
    private void onTextBufferMultiSet(PacketParser p, TextBuffer buffer) {
        try {
            int col = p.readInt();
            int row = p.readInt();
            String s = p.readUTF();
    ...
    private void onTextBufferRamInit(PacketParser p, TextBuffer buffer) {
        try {
    ...
    private void onTextBufferBitBlt(PacketParser p, TextBuffer buffer) {
        try {
            int col = p.readInt();
            int row = p.readInt();
            int w = p.readInt();
    ...
    private void onTextBufferRamDestroy(PacketParser p, TextBuffer buffer) {
        try {
            String owner = p.readUTF();
            int id = p.readInt();
    ...
    private void onTextBufferMultiRawSetText(PacketParser p, TextBuffer buffer) {
        try {
            int col = p.readInt();
            int row = p.readInt();
    ...
    private void onTextBufferMultiRawSetBackground(PacketParser p, TextBuffer buffer) {
        try {
            int col = p.readInt();
            int row = p.readInt();
            int maxRows = buffer.getMaximumHeight();
    ...
    private void onTextBufferMultiRawSetForeground(PacketParser p, TextBuffer buffer) {
        try {
            int col = p.readInt();
            int row = p.readInt();
            int maxRows = buffer.getMaximumHeight();
    ...
    private void onSound(PacketParser p) {
        try {
            ...
            li.cil.oc.fabric.util.Audio.play(x + 0.5f, y + 0.5f, z + 0.5f, frequency, duration);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private void onSoundPattern(PacketParser p) {
        try {
            ...
            li.cil.oc.fabric.util.Audio.play(x + 0.5f, y + 0.5f, z + 0.5f, pattern);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
