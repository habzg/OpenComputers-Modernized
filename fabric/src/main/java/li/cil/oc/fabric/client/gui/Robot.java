...
public class Robot extends DynamicGuiContainer<li.cil.oc.fabric.common.container.Robot> implements li.cil.oc.core.impl.client.gui.traits.InputBuffer {
    ...
    public Robot(Inventory playerInventory, li.cil.oc.fabric.common.blockentity.Robot robot) {
        this(new li.cil.oc.fabric.common.container.Robot(0, playerInventory, robot), playerInventory, Component.literal("Robot"));
    }

    public Robot(li.cil.oc.fabric.common.container.Robot container, Inventory inv, Component title) {
    ...
    private li.cil.oc.fabric.common.blockentity.Robot current() {
    ...
    private double bufferRenderWidth() {
        return Math.min(240.0, TextBufferRenderCache.renderer.charRenderWidth() * li.cil.oc.core.impl.OCSettings.screenResolutionsByTier[0][0]);
    }

    private double bufferRenderHeight() {
        return Math.min(140.0, TextBufferRenderCache.renderer.charRenderHeight() * li.cil.oc.core.impl.OCSettings.screenResolutionsByTier[0][1]);
    }
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float dt) {
        var r = current();
        powerButton.toggled = r.isRunning;
        scrollButton.active = canScroll();
        scrollButton.hoverOverride = isDragging;
        if (r.inventorySize < 16 + inventoryOffset * 4) scrollTo(0);
    ...
    private void onPowerButton(ImageButton ignoredButton) {
        var r = current();
        li.cil.oc.fabric.client.PacketSender.sendComputerPower(r, !r.isRunning);
    }
    ...
    protected void drawSecondaryForegroundLayer(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        if (powerButton.isMouseOver(mouseX, mouseY)) {
            List<Component> tooltip = new ArrayList<>();
            for (String line : (r.isRunning ? Component.translatable("gui.opencomputers.robot.turnoff").getString() : Component.translatable("gui.opencomputers.robot.turnon").getString()).split("\n")) {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float dt, int mouseX, int mouseY) {
        ...
        var r = current();
        power.level = r.globalBuffer / r.globalBufferSize;
