...
public final class HighlightRenderer {
    ...
    public static void init() {
        WorldRenderEvents.BLOCK_OUTLINE.register((context, outlineContext) -> {
            if (!(context.camera().getEntity() instanceof Player player)) return true;

            var level = player.level();
            var blockPos = outlineContext.blockPos();
            var hitResult = Minecraft.getInstance().hitResult;
            Direction sideHit = hitResult instanceof net.minecraft.world.phys.BlockHitResult blockHit ? blockHit.getDirection() : Direction.NORTH;

            if (li.cil.oc.api.Items.get(player.getItemInHand(InteractionHand.MAIN_HAND)) == tablet) {
                if (!level.isEmptyBlock(blockPos)) {
                    ...
                    if (bufferSource == null) return true;
            ...
            if (te instanceof Print print) {
                ...
                var bufferSource = context.consumers();
                if (pose == null || bufferSource == null) return true;
    ...
    private static int getCableConnections(net.minecraft.world.level.Level level, net.minecraft.core.BlockPos pos) {
        ...
        for (Direction side : Direction.values()) {
            ...
            if (neighbor != null && !(neighbor instanceof li.cil.oc.fabric.common.blockentity.RobotProxy)) {
